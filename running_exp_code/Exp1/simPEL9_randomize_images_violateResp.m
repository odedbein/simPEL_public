function simPEL9_randomize_images_violateResp(subj_num,subj_id)
...
if strcmp(hostname(1:6),'joanna')%the hostname command gives 1X7 char output, we only need the first 6.
    project_dir='/Volumes/Oded/Bein/simPEL/simPEL9_onlyConsSameSimilarHalf';
elseif strcmp(hostname(1:9),'electrode')%the hostname command gives 1X7 char output, we only need the first 6.
    project_dir='/Volumes/Oded/Bein/simPEL/simPEL9_onlyConsSameSimilarHalf';
else 
    project_dir='/Volumes/davachilab/Bein/simPEL/simPEL9_onlyConsSameSimilarHalf';
end
...
%% NEW IN THIS VERSION: now, just switch the response for the v items and lures:
stim_single_v_temp=[stim_single_v((pairs_per_cond/2+1):pairs_per_cond);stim_single_v(1:pairs_per_cond/2)];
