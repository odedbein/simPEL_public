function simPEL9_learning_day1(subj_num, subj_id,varargin)
...
if (mod(subj_num,2) == 0)
    instructString{1} = 'Decide whether each object is bigger or smaller than the previous object in real life';
...
part='day1_init';
load([data_dir '/trial_sequences.mat'],'stim_seq_day1','isi_day1','stim_loc_seq_day1','stim_in_pair_seq_day1','day1_init_onsets');
enc_idx=stim_seq_day1;
stim_in_pair=stim_in_pair_seq_day1;
loc_in_pair=stim_loc_seq_day1;
trial_onsets=day1_init_onsets;
isi_curr_part=isi_day1;
...
while repeat==1
    ...
    for rep=startBlock:n_enc_runs
        ...
        % if run # odd, have a short break:
        if (mod(rep,2) == 1) && (rep ~= n_enc_runs) && ~practice
         ...
         % if run 6, have a long break - go get the experimenter
        if (rep == 6) && ~practice
        ...
        end %ends the conditional for the sixth run

    end %ends the conditional for the the reps
end %ends the accuracy/repeat condition
