...
%all:
good_subj=1:numel(subjects_numbers);
...
%repetition:
AvAccResp=simPEL11_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.day1_init(good_subj,:);
...
%calculate the average accuracy for all items
AvAccResp=simPEL11_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.rem(good_subj,:);
%reported accuracy:
meanAvAccResp_perSubj=mean(AvAccResp,2);
...
%calculate the average accuracy for all items (excliude the col of 0's for
%second item in the violated pairs
AvAccResp=simPEL11_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.er_block(good_subj,[1:5 7:10]);

%reported accuracy:
meanAvAccResp_perSubj=mean(AvAccResp,2);
fprintf('accuracy during violation: M = %.2f, SD: %.2f\n',mean(meanAvAccResp_perSubj),std(meanAvAccResp_perSubj));
%compare violation and no violation:
fprintf('accuracy v items: M = %.2f, SD: %.2f\n',mean(AvAccResp(:,8)),std(AvAccResp(:,8)));
fprintf('accuracy nv items: M = %.2f, SD: %.2f\n',mean(AvAccResp(:,9)),std(AvAccResp(:,9)));
fprintf('accuracy violation: v vs. nv items: \n');
[h,p,ci,stats] = ttest(AvAccResp(:,8),AvAccResp(:,9))
