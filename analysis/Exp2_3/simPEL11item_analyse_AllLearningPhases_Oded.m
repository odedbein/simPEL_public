function [data_learning, simPEL11_All_subs_learning_byItemAllReps,simPEL11_All_subs_learning, simPEL11_All_subs_learning_averages,simPEL11_All_subs_learning_byItemAllRepsCollapsVnv]=simPEL11item_analyse_AllLearningPhases_Oded() %subjects_numbers,subjects_names
...
simPEL11_All_subs_learning={};
simPEL11_All_subs_learning_averages={};
simPEL11_All_subs_learning_byItemAllRepsCollapsVnv={};
simPEL11_All_subs_learning_byItemAllReps={};
    ...
    for part=1:numel(part_name)
        ...
        for rep=1:rep_num
            ...
            for subj=1:length(subjects_numbers)
                subj_num=subjects_numbers(subj);
                subj_name=['S' num2str(subj_num) subjects_names{subj}];
                simPEL11_All_subs_learning.accuracy_num.(ABmem_name{bb}).(part_name{part}).(rep_name)(subj,:)=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).accuracy_num(rep,:);
                simPEL11_All_subs_learning.accuracy_rates.(ABmem_name{bb}).(part_name{part}).(rep_name)(subj,:)=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).accuracy_rates(rep,:);
                simPEL11_All_subs_learning.RT.(ABmem_name{bb}).(part_name{part}).(rep_name)(subj,:)=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).RT(rep,:);
                simPEL11_All_subs_learning.logRT.(ABmem_name{bb}).(part_name{part}).(rep_name)(subj,:)=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).logRT(rep,:);
            end
            simPEL11_All_subs_learning_averages.accuracy_num.(ABmem_name{bb}).(part_name{part})(rep,:)=mean(simPEL11_All_subs_learning.accuracy_num.(ABmem_name{bb}).(part_name{part}).(rep_name));
            simPEL11_All_subs_learning_averages.accuracy_rates.(ABmem_name{bb}).(part_name{part})(rep,:)=mean(simPEL11_All_subs_learning.accuracy_rates.(ABmem_name{bb}).(part_name{part}).(rep_name));
            simPEL11_All_subs_learning_averages.RT.(ABmem_name{bb}).(part_name{part})(rep,:)=mean(simPEL11_All_subs_learning.RT.(ABmem_name{bb}).(part_name{part}).(rep_name));
            simPEL11_All_subs_learning_averages.logRT.(ABmem_name{bb}).(part_name{part})(rep,:)=mean(simPEL11_All_subs_learning.logRT.(ABmem_name{bb}).(part_name{part}).(rep_name));
        end
    ...
    for bb=1:numel(ABmem_name)
        for part=1:2
            ...
            for loc=1:prev_chunk_size
                ...
                simPEL11_All_subs_learning_byItemAllRepsCollapsVnv.accuracy_num.(ABmem_name{bb}).(part_name{part})(subj,((loc-1)*rep_num)+(1:rep_num))=curr_trials';
                curr_trials=mean(data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).accuracy_rates(:,[loc,loc+prev_chunk_size]),2);
                simPEL11_All_subs_learning_byItemAllRepsCollapsVnv.accuracy_rates.(ABmem_name{bb}).(part_name{part})(subj,((loc-1)*rep_num)+(1:rep_num))=curr_trials';
                curr_trials=mean(data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).RT(:,[loc,loc+prev_chunk_size]),2);
                simPEL11_All_subs_learning_byItemAllRepsCollapsVnv.RT.(ABmem_name{bb}).(part_name{part})(subj,((loc-1)*rep_num)+(1:rep_num))=curr_trials';
                curr_trials=mean(data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).logRT(:,[loc,loc+prev_chunk_size]),2);
                simPEL11_All_subs_learning_byItemAllRepsCollapsVnv.logRT.(ABmem_name{bb}).(part_name{part})(subj,((loc-1)*rep_num)+(1:rep_num))=curr_trials';
            end
        ...
        for part=1:3
            ...
            for item=1:curr_size(2) %for each item
                num_rep=curr_size(1);
                simPEL11_All_subs_learning_byItemAllReps.accuracy_num.(ABmem_name{bb}).(part_name{part})(subj,(item-1)*num_rep+(1:num_rep))=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).accuracy_num(:,item)';
                simPEL11_All_subs_learning_byItemAllReps.accuracy_rate.(ABmem_name{bb}).(part_name{part})(subj,(item-1)*num_rep+(1:num_rep))=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).accuracy_rates(:,item)';
                simPEL11_All_subs_learning_byItemAllReps.RT.(ABmem_name{bb}).(part_name{part})(subj,(item-1)*num_rep+(1:num_rep))=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).RT(:,item)';
                simPEL11_All_subs_learning_byItemAllReps.logRT.(ABmem_name{bb}).(part_name{part})(subj,(item-1)*num_rep+(1:num_rep))=data_learning.(subj_name).(ABmem_name{bb}).(part_name{part}).logRT(:,item)';
            end
...
for part=1:numel(part_name)
    ...
    for rep=1:size(response,1)
        ...
        %curr_RTs(loc_in_pair(rep,:)==1)=0; %Here we do take A items - since it's an item-focused task. null A items - don't want to consider them because they are at the begining of a pair and the big/small responses are not controlled for.
    ...
    for bb = 1:numel(ABmem_name)
        ...
        for cond=1:num_triads_conds
            ...
            for loc=curr_loc %for all locs
                ...
                %consider accuracy for all items, because it's an item focused task here (not like other versions)            
