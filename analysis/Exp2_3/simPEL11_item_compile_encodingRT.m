function data_learning=simPEL11_item_compile_encodingRT(engram)
if engram
    mydir='/data/Bein';
else
...
for part=1:numel(part_name)
    ...
    %write it up:
    results_fname=['simPEL11_item_encoding_' part_name{part} '.txt'];
...
for part=1:numel(part_name)
    ...
    if all(isnan(resp_bg_col))==1
        fprintf('subj %d%s: smt is wrong, no bg responses\n',subj_num, subj_id)
    end
