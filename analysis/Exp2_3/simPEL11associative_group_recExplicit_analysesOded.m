...
%associative memory:
%below 40% in both conditions: 
good_subj=[1:2 4:7 9:15 17:21 23:32]; 

%analyze accuracy:
data=simPEL11_All_subs_recognition.All.accuracy_rates;
...
data=simPEL11_All_subs_recognition.All.accuracy_rates(good_subj,:);
...
title('simPEL11 Recognition','Fontsize',24);
...
%analyze accuracy:
data=simPEL11_All_subs_recognition.All.response_typePerCond(:,[3 2 6 5 9 8 12 11 15 14]); %remove "new" responses, change the order of similar/old
header.rec(1,2:end+1)=header.rec(1,1:end);
header.rec{1,1}='subjects';
AccStruct.rec=header.rec;
AccStruct.rec(2:length(subjects_numbers)+1,1)=num2cell(subjects_numbers');
...
data=data(good_subj,:);
%data=data([1:17 19:end],:); % use if you want to remove the subject with perfect AB memory
...
set(gca, 'FontSize', 14)
ylabel('accuracy','Fontsize',16);   
title('simPEL11 by halves - all response types','Fontsize',24);
...
set(gca, 'FontSize', 14)
ylabel('accuracy','Fontsize',16);   
title('simPEL11 - similar/old responses','Fontsize',24);
...
%% stats:
data=curr_data; 
%within old items - old vs. similar responses, violation vs. no violation
curr_data=data(:,1:4);
...
col1=1;
col2=2;
fprintf('old items: old responses v/nv \n');
[h,p,ci,stats] = ttest(curr_data(:,col1),curr_data(:,col2));
p

col1=3;
col2=4;
fprintf('old items: similar responses v/nv  \n');
[h,p,ci,stats] = ttest(curr_data(:,col1),curr_data(:,col2));
p
...
F1=[ones(n*num_cond,1);ones(n*num_cond,1)*2];%old/similar
F2=reshape(repmat(1:num_cond,n,2),size(Y));%violation/no-violation
fprintf('ANOVA old responses: F1:item type: old/similar, F2: violation/no-violation \n');
X=[Y,F1,F2,S];
RMAOV2_mod(X,alpha,showtable);
...
data=simPEL11_All_subs_recognition.All.Bmem.response_typePerCond(:,choose_conds); %remove "new" responses, change the order of similar/old
header.rec(1,2:end+1)=header.rec(1,1:end);
header.rec{1,1}='subjects';
AccStruct.rec=header.rec;
AccStruct.rec(2:length(subjects_numbers)+1,1)=num2cell(subjects_numbers');
...
set(gca, 'FontSize', 14)
ylabel('accuracy','Fontsize',16);   
title('simPEL11 - all response types','Fontsize',24);
...
fprintf('Brem: similar items: old responses: v/nv \n');
col1=3;
col2=4;
[~,p,ci,stats] = ttest(stats_data(:,col1),stats_data(:,col2));
CohenD=nanmean(stats_data(:,col1)-stats_data(:,col2))/nanstd(stats_data(:,col1)-stats_data(:,col2));
...
%one subject has NaN bc he remembered everything, so exclude
%him:
stats_data=curr_data([1:17 19:end],[3:4 7:8]);
...
F1=[ones(n*num_cond,1);ones(n*num_cond,1)*2];%old/similar
F2=reshape(repmat(1:num_cond,n,2),size(Y));%violation/no-violation
fprintf('ANOVA B-forg, old items: F1:response type: old/similar, F2: violation/no-violation \n');
X=[Y,F1,F2,S];
RMAOV2_mod_oded(X,alpha,showtable);
...
stats_data=curr_data([1:17 19:end],[1,3,5,7]);
...
%% recognition - analyse all response types B item memory don't plot forgotten items:
plotSubj=0;
%that's like above - 28 subj
good_subj=[1:2 4:7 9:15 17:21 23:32]; 
...
col1=3;
col2=4;
fprintf('B-rem: old-items, similar responses: v/nv  \n');
[h,p,ci,stats] = ttest(curr_data(:,col1),curr_data(:,col2));
p
...
% calculate and plot the "discrimination score" - saying old on old vs. saying old on similar.
disc_score=[curr_data(:,1)-curr_data(:,5) curr_data(:,2)-curr_data(:,6)];
%ttest:
col1=1;
col2=2;
fprintf('B-rem, old responses, discrimination score (saying "old" on old vs. similar items): v/nv \n');
[h,p,ci,stats] = ttest(disc_score(:,col1),disc_score(:,col2));
p
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% analyze explicit - part1
plotSubj=0;
%analyze old/new
data=simPEL11_All_subs_explicit.Part1.OldNewAvDist;
...
data=simPEL11_All_subs_explicit.Part1.OldNewAvDist(good_subj,:);
...
data=simPEL11_All_subs_explicit.Part1.CountEachRatingAvDist(good_subj,:);
color=[0        0.4470    0.7410
       0.8500   0.3250    0.0980
       0.8      0.8       0.8
       0.8      0.8       0.8];
...
%% analyze explicit - PART2
plotSubj=0;
%analyze old/new
data=simPEL11_All_subs_explicit.Part2.OldNewAvDist(good_subj,:);
...
chance=ones(numel(good_subj),1)*.33;
fprintf('v/chance t-tests \n');
[h,p,ci,stats]=ttest(curr_data(:,1),chance)

fprintf('nv/chance t-tests \n');
[h,p,ci,stats]=ttest(curr_data(:,2),chance)


%% analyze average confidence rates:
data=simPEL11_All_subs_explicit.Part2.AvConfRatesAvDist(good_subj,:);
color=[0        0.4470    0.7410
       0.8500   0.3250    0.0980
       0.8      0.8       0.8
       0.8      0.8       0.8];
...
ylim([1 6]);
Xticks={'AB: violation-hits','AB: no-violation-hits','AB: violation-foils','AB: no-violation-foils'};
set(gca,'XTickLabel',Xticks,'XTickLabelRotation',25)
set(gca, 'FontSize', 14)
title('AB memory: average confidence','FontSize', 18)
...
%% analyze count confidence rates:
data=simPEL11_All_subs_explicit.Part2.CountEachRatingAvDist(good_subj,:);
color=[0        0.4470    0.7410
       0.8500   0.3250    0.0980
       0.8      0.8       0.8
       0.8      0.8       0.8];
