...
%% only accuracy was reported based on this script - all stats are bsed on the R code.
%% but there are some plots here of the learning, might be useful
...
%subj num 35 was excluded due to technical error
...
%% day1_init:
%% create data structures with headers that are easy to look at - all participants:
...
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.AllItems.day1_init(good_subj,:);
else
    RT=simPEL9_All_subs_learning_byItemAllReps.RT.AllItems.day1_init(good_subj,:);
end
...
AccStruct.day1_init(2:length(good_subj)+1,2:end)=num2cell(simPEL9_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.day1_init(good_subj,:));

%calculate the average accuracy for all relevant items (B) for each
%repetition:
AccResp=simPEL9_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.day1_init(good_subj,:);
AvAccResp=[];
for rep=1:num_reps_init
...
%if we do ttest for each repetition, SEM should be computed per repetition. if I put it all in an ANOVA, then
%across all repetitions. for now, it's per repetition
...
if logRT
    curr_data=simPEL9_All_subs_learning_byItemAllRepsCollapsVnv.logRT.AllItems.day1_init(good_subj,:);
else
    curr_data=simPEL9_All_subs_learning_byItemAllRepsCollapsVnv.RT.AllItems.day1_init(good_subj,:);
end
...
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.AllItems.rem(good_subj,:);
else
    RT=simPEL9_All_subs_learning_byItemAllReps.RT.AllItems.rem(good_subj,:);
end
RTStruct.rem=header.rem;
RTStruct.rem(2:length(good_subj)+1,1)=num2cell(subjects_numbers(good_subj)');
RTStruct.rem(2:length(good_subj)+1,2:end)=num2cell(RT);
%accuracy structure:
AccStruct.rem=RTStruct.rem;
AccStruct.rem(2:length(good_subj)+1,2:end)=num2cell(simPEL9_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.rem(good_subj,:));

%calculate the average accuracy for all relevant items (B)
AvAccResp=simPEL9_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.rem(good_subj,[2,4]);
...
%compute SEM:
%calculate the SEM for error bars - based on a ttest for each pair
plotSubj=0;
%good_subj=1:length(subjects_numbers); 
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.AllItems.rem(good_subj,:);
...
title('simPEL9 reminder','Fontsize',20);
...
logRT=1;
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.AllItems.er_block(good_subj,:);
else
    RT=simPEL9_All_subs_learning_byItemAllReps.RT.AllItems.er_block(good_subj,:);
end
RTStruct.er_block=header.er_block;
RTStruct.er_block(2:length(good_subj)+1,1)=num2cell(subjects_numbers(good_subj)');
RTStruct.er_block(2:length(good_subj)+1,2:end)=num2cell(RT);
%accuracy structure:
AccStruct.er_block=RTStruct.er_block;
AccStruct.er_block(2:length(good_subj)+1,2:end)=num2cell(simPEL9_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.er_block(good_subj,:));

%calculate the average accuracy for all relevant items (B)
AvAccResp=simPEL9_All_subs_learning_byItemAllReps.accuracy_rate.AllItems.er_block(good_subj,[2,4,8:10]);
...
title('simPEL9 v/nv block: reminder','Fontsize',20);
Xticks={'violation: A','violation: B','no-violation: A','no-violation: B'};
set(gca,'XTickLabel',Xticks,'FontSize', 14,'XTickLabelRotation',25);
...
title('simPEL9 v/nv block: violation','Fontsize',20);
...
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.Brem.day1_init(good_subj,:);
    RT=[RT simPEL9_All_subs_learning_byItemAllReps.logRT.Bforg.day1_init(good_subj,:)];
else
    RT=simPEL9_All_subs_learning_byItemAllReps.RT.Brem.day1_init(good_subj,:);
    RT=[RT simPEL9_All_subs_learning_byItemAllReps.RT.Bforg.day1_init(good_subj,:)];
end
RTStruct.day1_init=header.day1_init;
RTStruct.day1_init(2:length(good_subj)+1,1)=num2cell(subjects_numbers(good_subj)');
RTStruct.day1_init(2:length(good_subj)+1,2:end)=num2cell(RT);
...
logRT=0;
if logRT
    curr_data=simPEL9_All_subs_learning_byItemAllRepsCollapsVnv.logRT.Brem.day1_init(good_subj,:);
    curr_data=[curr_data simPEL9_All_subs_learning_byItemAllRepsCollapsVnv.logRT.Bforg.day1_init(good_subj,:)];
else
    curr_data=simPEL9_All_subs_learning_byItemAllRepsCollapsVnv.RT.Brem.day1_init(good_subj,:);
    curr_data=[curr_data simPEL9_All_subs_learning_byItemAllRepsCollapsVnv.RT.Bforg.day1_init(good_subj,:)];
end
...
%good_subj=1:length(subjects_numbers); 
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.Brem.rem(good_subj,:);
    RT=[RT simPEL9_All_subs_learning_byItemAllReps.logRT.Bforg.rem(good_subj,:)];
else
    RT=simPEL9_All_subs_learning_byItemAllReps.RT.Brem.rem(good_subj,:);
    RT=[RT simPEL9_All_subs_learning_byItemAllReps.RT.Bforg.rem(good_subj,:)];
end
...
%use this if you want to remove the participant from the graphs:
%curr_data=RT([1:24 26:end],shuffle_cond);
...
title('simPEL9 reminder, by ABrem/forg','Fontsize',20);
...
stats_data=curr_data([1:24 26:end],1:4);
...
if logRT
    RT=simPEL9_All_subs_learning_byItemAllReps.logRT.Brem.er_block(good_subj,:);
    RT=[RT simPEL9_All_subs_learning_byItemAllReps.logRT.Bforg.er_block(good_subj,:)];
else
    RT=simPEL9_All_subs_learning_byItemAllReps.RT.Brem.er_block(good_subj,:);
    RT=[RT simPEL9_All_subs_learning_byItemAllReps.RT.Bforg.er_block(good_subj,:)];
end
...
%curr_data=RT([1:24 26:end],select_cond(shuffle_cond));
num_subj=size(curr_data,1);
meanRT=nanmean(curr_data);
%compare within condition, A vs. B:
for i=[1,5]
...
%remove one ss with nans:
stats_data=curr_data([1:24 26:end],5:8);
...
title('simPEL9 v/nv block ABrem/forg: violation','Fontsize',20);
set(gca,'XTickLabel',Xticks,'FontSize', 14,'XTickLabelRotation',25);
...
%remove one ss with nans:
curr_data=curr_data([1:24 26:end],:);
